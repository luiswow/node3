   ...
   authenticate: function(req, res, next) {
      Usuario.findOne({email: req.body.email}, (err, userInfo) => {
         if (err) {
            ...
            if (userInfo === null) { return res.status(401).json({status: "error", message: "Email/Password inválido", data: null}); }
            if (userInfo != null && bcrypt.compareSync(req.body.password, userInfo.password)){
               const token = jwt.sign({id: userInfo._id}, req.app.get('secretKey'), { expiresIn: '7d' });
               res.status(200).json({message: 'Usuario encontrado!', data: { usuario: userInfo, token: token }});
            } else {
               res.status(401).json({status: "error", message: 'Email/Password inválido', data: null});
            }
         }
      });
   },
